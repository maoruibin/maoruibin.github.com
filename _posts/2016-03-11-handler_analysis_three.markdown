...
其实在Handler 之 源码分析一文中，关于 Handler 有一点我一直没说到。

Handler创建的时候必须使用当前线程的 Looper 来构造消息循环系统，而自己手动创建的子线程默认是没有 Looper 的，
如果在一个子线程中创建 Handler ，就必须为这个子线程创建 Looper，否则我们就会看到一个常见的异常
...
首先分析出错的情形，也就是直接在子线程中 new Handler() 时报错，是怎么发生的。现在子线程中创建一个 Handler
...
然后，在以前，我直接在 handler 之前根据错误提示加一个 Looper.prepare() 方法，然后错误就没有了，我自己也就不管了，但是这次，我一定要看看，为什么调用 Looper.prepare() 就不报错了。
...
此时在方法中，首先判断 sThreadLocal.get() != null ，因为现在在子线程，而且没有为 sThreadLocal 设置过 Looper 对象，所以此时
...
因为主线程，也就是我们经常提到的主线程，也叫UI线程，它其实就是ActivityThread，ActivityThread被创建时就会初始化Looper，代码如下

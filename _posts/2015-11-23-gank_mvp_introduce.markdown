...
tags: Android App
---
最近完成了一个干货客户端，整个客户端借鉴 drakeet 的[妹纸](https://github.com/drakeet/Meizhi)。但是在原项目的基础上，自己使用 MVP 的结构模式
对项目进行重构，让项目解耦，使业务逻辑和视图表现层相分离，从而让代码逻辑变得更简单，以下就整理了自己在重构项目过程中对 MVP 这种模式
的理解以及项目中如何实施 MVP 的模式。

###关于MVP

MVP 是 Module-Presenter-View的缩写，翻译过来就是模型-控制器-视图，是一种流行的开发架构模式。他主张让 Presenter 控制复杂的业务逻辑，
...
自己一开始写代码时，确实也是这样写的，记得曾经写过一个详情展示页，代码写了2000多行，当时觉得还挺自豪，一个类2000多行呢，现在想想也是觉得
 too young to simple .
 
 按照 MVP 的定义，我们应该把所有的业务逻辑操作都写在 Presenter 层，而 View 层，也就是我们的 Activity、Fragment,他们主要做一些更新界面、
 向 Presenter 层发送命令的操作。Module 层则主要负责具体的数据获取操作。他们的具体关系可以看下面这张图。
 
 ![mvp](/assets/gank_mvp_1.jpg "mvp")
 
 由图可以清楚的看到，Presenter 处在一个中心位置，View 层向 Presenter 发送请求，Presenter 自己接受请求，但是自己不具体处理请求，而是将
 具体的事情交给 Module 去处理，利用 Module 请求完毕，Presenter 再把具体的结果响应在 View 层。
 
 ####一个例子
 
 这个过程其实理解的更加形象一点。打个比喻，把 P 理解为自己的头部，V 理解为右手 M 理解为 左手,假设左右手不能直接沟通，左右手分别通过胳膊
 和头部联系，现在 V(右手)需要100块钱进行消费，但是钱却在 M(左手)里，左手有大把的钱，这时 V 通过右胳膊告诉头部，说自己需要100块钱。头部
  得到命令后，自己没钱，只得去请求 M (左手),左手知道 P 需要钱，所以就慷慨的给钱，尽管他不知道 P 会怎么用这笔钱，P 拿到钱后，
  首先用验钞机验一下，发现是真的，然后就把钱给了 V（右手），此时头部需要调用 V(右手) 的一个方法，这个方法叫 hold() 用来操作右手，让他
  攥紧钱，这样钱就不容易被别人拿走了。到这里，整个过程就结束了，M 自始至终也没有跟 V 发生任何关系，他甚至不知道 V 的存在。M 则在
  最中央的位置负责协调数据、操作View。整个 MVP 的大体结构就是这样。
 ...
 * 在 View 层中怎么去调用 Presenter 中的方法？
 
 这个简单，在View初始化时，new 一个 Presenter 实例不就对了吗，是的可行，我也是这样做的。
 
 * Presenter 怎么去控制 View 中的界面逻辑？具体点就是，上面的头部最后怎么调用右手的 hold() 方法？
 
 这里需要使用接口回调实现，每一个 View 类对应一个抽象的接口，也就是说在写每个 Activity 时，首先把具体的
